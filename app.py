...
from utils import create_dataframe, extract_features, create_batch, \
    create_index, save_index, load_index, load_df
import yaml
import logging
...
class ImageSearchApp:
    ...
    def search_image(self, image_path):
        ...
        try:
            embeds = extract_features(
                image_path, self.image_processor, self.model)
    ...
    def run(self):
        ...
        search_path = st.sidebar.text_input(
            "Search folder path", help="Initilize the first search for creation of index and dataframe").replace('"', "")
        ...
        loaded_index_path = load_form.text_input("Index path").replace('"', "")
        loaded_df_path = load_form.text_input(
            "Dataframe path").replace('"', "")
        ...
        if submitted:
            with st.spinner():
                ...
                st.sidebar.success(
                    f"{len(st.session_state.dataframe)} images loaded.")
                logger.info(
                    f"{len(st.session_state.dataframe)} images loaded.")
                st.session_state.can_search = True
